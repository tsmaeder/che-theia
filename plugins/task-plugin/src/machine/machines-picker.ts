...
export class MachinesPicker {
    ...
    protected async doPick(hideToolingContainers: boolean): Promise<string> {
        ...
        return new Promise<string>(resolve => {
            ...
            for (const machineName of containers) {
                items.push(new ContainerItem(machineName));
            }
    ...
    protected async getContainers(hideToolContainers: boolean): Promise<string[]> {
        ...
        for (const machineName in containers) {
            if (!containers.hasOwnProperty(machineName)) {
                continue;
            }

            if (hideToolContainers && !this.isToolingContainer(containers[machineName])) {
                continue;
            }

            filteredContainers.push(machineName);
        }
